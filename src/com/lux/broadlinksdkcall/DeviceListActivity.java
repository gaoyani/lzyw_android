...
public class DeviceListActivity extends Activity {
	...
	private Button mBtnInitButton, mBtnProbeListButton, mBtnSwitchOn,
			mBtnSwitchOff, mBtnEasyConfigV2, mBtnEasyConfigV1;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_broadlink_device_list);
		findView();
		setListener();
		...
		mLvDeviceList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (mDeviceList.get(position).type.equals(DeviceType.A1)) {
					 ...
					 * A1InfoActivity.class); intent.putExtra("A1_Mac",
					 * mDeviceList.get(position).mac); addDevice(position);
					 * startActivity(intent);
					 */
				} else {
					Toast.makeText(context,
							"toast_demo_is_under_vevision",
	...
	public void probeList() {
		...
		String probeOut;
		in.addProperty(SysApplication.api_id, 11);
		in.addProperty(SysApplication.command, "probe_list");
		String string = in.toString();
		probeOut = SysApplication.mBlNetwork.requestDispatch(string);

		out = new JsonParser().parse(probeOut).getAsJsonObject();
		int code = out.get(CODE).getAsInt();
		String msg = out.get(MSG).getAsString();
		Log.e("result", "probeOut " + probeOut + "code  " + code + "msg  "
				+ msg);
		...
		mDeviceList = (ArrayList<DeviceInfo>) gson.fromJson(listJsonArray,
				listType);
	...
	public void addDevice(int position) {
		JsonObject in = new JsonObject();
		JsonObject out = new JsonObject();
		in.addProperty(SysApplication.api_id, 12);
		in.addProperty(SysApplication.command, "device_add");
		...
		String string = in.toString();
		String outString;
		outString = SysApplication.mBlNetwork.requestDispatch(string);
