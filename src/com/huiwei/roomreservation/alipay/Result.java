...
public class Result {
	...
	public  void parseResult() {
		...
		try {
			String src = mResult.replace("{", "");
			src = src.replace("}", "");
			resultKey = getContent(src, "resultStatus=", ";memo");
			if (sResultStatus.containsKey(resultKey)) {
				resultStatus = sResultStatus.get(resultKey);
			} else {
				resultStatus = "其他错误";
			}
			resultStatus += "(" + resultKey + ")";
	...
	private  boolean checkSign(String result) {
		...
		try {
			...
			if (signType.equalsIgnoreCase("RSA")) {
				retVal = Rsa.doCheck(signContent, sign, Keys.PUBLIC.replaceAll("\"", ""));
			}
		} catch (Exception e) {
			e.printStackTrace();
